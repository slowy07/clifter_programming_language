...
class Lexer:
  ...
  def make_tokens(self):
    ...
    while self.current_char != None:
      if self.current_char in ' \t':
        ...
        tokens.append(Token(CLF_MUL, pos_start=self.pos))
        self.advance()
      elif self.current_char == '/':
        tokens.append(Token(CLF_DIV, pos_start=self.pos))
        self.advance()
      elif self.current_char == '^':
        tokens.append(Token(CLF_POW, pos_start=self.pos))
        self.advance()
      elif self.current_char == '(':
        tokens.append(Token(CLF_LPAREN, pos_start=self.pos))
        self.advance()
      elif self.current_char == ')':
        tokens.append(Token(CLF_RPAREN, pos_start=self.pos))
        self.advance()
      elif self.current_char == '[':
        tokens.append(Token(CLF_LSQUARE, pos_start=self.pos))
        self.advance()
      elif self.current_char == ']':
        ...
        tokens.append(Token(CLF_COMMA, pos_start=self.pos))
        self.advance()
      else:
        pos_start = self.pos.copy()
        char = self.current_char
    ...
    tokens.append(Token(CLF_EOF, pos_start=self.pos))
    return tokens, None
  ...
  def make_identifier(self):
    ...
    tok_type = CLF_KEYWORD if id_str in KEYWORDS else CLF_IDENTIFIER
  ...
  def make_not_equals(self):
    ...
    if self.current_char == '=':
      self.advance()
      return Token(CLF_NE, pos_start=pos_start, pos_end=self.pos), None
...
class Parser:
  ...
  def parse(self):
    res = self.statements()
    if not res.error and self.current_tok.type != CLF_EOF:
  ...
  def statements(self):
    ...
    while self.current_tok.type == CLF_NEWLINE:
  ...
  def statement(self):
    ...
    if self.current_tok.matches(CLF_KEYWORD, 'return'):
      res.register_advancement()
      self.advance()
    ...
    if self.current_tok.matches(CLF_KEYWORD, 'continue'):
      res.register_advancement()
      self.advance()
      return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copy()))
      
    if self.current_tok.matches(CLF_KEYWORD, 'break'):
      res.register_advancement()
      self.advance()
  ...
  def expr(self):
    ...
    if self.current_tok.matches(CLF_KEYWORD, 'var'):
      res.register_advancement()
      self.advance()
      ...
      if self.current_tok.type != CLF_EQ:
        return res.failure(InvalidSyntaxError(
          self.current_tok.pos_start, self.current_tok.pos_end,
          "Expected '='"
        ))
  ...
  def comp_expr(self):
    ...
    if self.current_tok.matches(CLF_KEYWORD, 'not'):
  ...
  def arith_expr(self):
    return self.bin_op(self.term, (CLF_PLUS, CLF_MINUS))

  def term(self):
    return self.bin_op(self.factor, (CLF_MUL, CLF_DIV))

  def factor(self):
    res = ParseResult()
    tok = self.current_tok

    if tok.type in (CLF_PLUS, CLF_MINUS):
      res.register_advancement()
      self.advance()
  ...
  def power(self):
    return self.bin_op(self.call, (CLF_POW, ), self.factor)
  ...
  def atom(self):
    ...
    if tok.type in (CLF_INT, CLF_FLOAT):
      res.register_advancement()
      self.advance()
      return res.success(NumberNode(tok))

    elif tok.type == CLF_STRING:
      res.register_advancement()
      self.advance()
      return res.success(StringNode(tok))

    elif tok.type == CLF_IDENTIFIER:
      res.register_advancement()
      self.advance()
      return res.success(VarAccessNode(tok))

    elif tok.type == CLF_LPAREN:
      res.register_advancement()
      self.advance()
      expr = res.register(self.expr())
      if res.error: 
  ...
  def list_expr(self):
    ...
    if self.current_tok.type != CLF_LSQUARE:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected '['"
      ))
  ...
  def if_expr_c(self):
    ...
    if self.current_tok.matches(CLF_KEYWORD, 'else'):
      res.register_advancement()
      self.advance()
  ...
  def if_expr_cases(self, case_keyword):
    ...
    if not self.current_tok.matches(CLF_KEYWORD, case_keyword):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
  ...
  def for_expr(self):
    ...
    if not self.current_tok.matches(CLF_KEYWORD, 'for'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'for'"
      ))
  ...
  def while_expr(self):
    ...
    if not self.current_tok.matches(CLF_KEYWORD, 'while'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'while'"
      ))
  ...
  def func_def(self):
    ...
    if not self.current_tok.matches(CLF_KEYWORD, 'fun'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'fun'"
      ))
    ...
    if self.current_tok.type == CLF_ARROW:
      res.register_advancement()
      self.advance()
...
class Interpreter:
  ...
  def visit_BinOpNode(self, node, context):
    ...
    elif node.op_tok.type == CLF_MINUS:
      result, error = left.subbed_by(right)
    elif node.op_tok.type == CLF_MUL:
      result, error = left.multed_by(right)
    elif node.op_tok.type == CLF_DIV:
      result, error = left.dived_by(right)
    elif node.op_tok.type == CLF_POW:
      result, error = left.powed_by(right)
    elif node.op_tok.type == CLF_EE:
      result, error = left.get_comparison_eq(right)
    elif node.op_tok.type == CLF_NE:
      result, error = left.get_comparison_ne(right)
    elif node.op_tok.type == CLF_LT:
      result, error = left.get_comparison_lt(right)
    elif node.op_tok.type == CLF_GT:
      result, error = left.get_comparison_gt(right)
    elif node.op_tok.type == CLF_LTE:
      result, error = left.get_comparison_lte(right)
    elif node.op_tok.type == CLF_GTE:
      result, error = left.get_comparison_gte(right)
    elif node.op_tok.matches(CLF_KEYWORD, 'and'):
      result, error = left.anded_by(right)
    elif node.op_tok.matches(CLF_KEYWORD, 'or'):
  ...
  def visit_UnaryOpNode(self, node, context):
    ...
    if node.op_tok.type == CLF_MINUS:
      number, error = number.multed_by(Number(-1))
    elif node.op_tok.matches(CLF_KEYWORD, 'not'):
